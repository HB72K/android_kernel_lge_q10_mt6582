...
int iWriteEEPROM_NU(u16 a_u2Addr  , u32 a_u4Bytes, u8 * puDataInBytes)
{
    int  i4RetValue = 0;
    u32 u4Index = 0;
...
int iReadEEPROM_NU(u16 a_u2Addr, u32 ui4_length, u8 * a_puBuff)
{
    int  i4RetValue = 0;
...
int iWriteData_NU(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
...
int iReadDataFromBRCC064GWZ_3(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
...
int iReadData_NU(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   int  i4RetValue = 0;
   int  i4ResidueDataLength;
   u32 u4IncOffset = 0;
...
//   EEPROMDB("[S24EEPORM] iReadData \n" );
   ...
   do 
   {
       ...
       {
           i4RetValue = iReadEEPROM_NU((u16)u4CurrentOffset, 8, pBuff);
           if (i4RetValue != 0)
           {
                EEPROMDB("[BRCC064GWZ_3_eeprom] I2C iReadData failed!! \n");
                return -1;
       ...
       {
           i4RetValue = iReadEEPROM_NU((u16)u4CurrentOffset, i4ResidueDataLength, pBuff);
           if (i4RetValue != 0)
           {
                EEPROMDB("[BRCC064GWZ_3_eeprom] I2C iReadData failed!! \n");
                return -1;
...
#endif//COMMON_CAM_CAL_DRV
{
    ...
    switch(a_u4Command)
    {
...
#endif            
            i4RetValue = iWriteData_NU((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
#ifdef EEPROMGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            ...
            else if(ptempbuf->u4Offset == 0x000CC064)
            {
                *(u32 *)pWorkingBuff = 0x001CC064;
            }
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000BCC64) && (EEPROM_Address[0]==0x0))
            {
            ...
            else
            {
               ...
               {               	 
                   i4RetValue =  iReadDataFromBRCC064GWZ_3(ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff);
...
#if defined( COMMON_CAM_CAL_DRV)
EXPORT_SYMBOL(BRCC064GWZ_3_EEPROM_Ioctl);
#else
