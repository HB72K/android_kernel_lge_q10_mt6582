...
int fgauge_get_saddles_align(void)
{
	return DOD_ALIGNMEMT_SET+1;
}
...
kal_int32 fgauge_read_capacity_by_v(kal_int32 voltage)
{
	...
	saddles = fgauge_get_saddles_align();
...
void fgauge_construct_battery_profile_init(void)
{
	...
	temp_profile_p = (BATTERY_PROFILE_STRUC_P) kmalloc((DOD_ALIGNMEMT_SET+1) * sizeof(*temp_profile_p), GFP_KERNEL);
	memset(temp_profile_p, 0, (DOD_ALIGNMEMT_SET+1) * sizeof(*temp_profile_p));
	for (i=0; i< PROFILE_SIZE; i++) {
		profile_index = 0;
		for (j=0; j*2 <= (DOD_ALIGNMEMT_SET*2); j++) {
...
void fgauge_construct_battery_profile(kal_int32 temperature, BATTERY_PROFILE_STRUC_P temp_profile_p)
{
	...
	saddles = fgauge_get_saddles();
	memset(temp_profile_p, 0, saddles * sizeof(*temp_profile_p));
...
void dod_init(void)
{
	...
	if (g_rtc_fg_soc != 0)
	{
		...
		/* if battery is full, use current soc to make soc over 100 */
		if (g_rtc_fg_soc == 100 && gFG_capacity_by_v > 100) {
			use_rtc_fg_soc = KAL_FALSE;
		}
		...
		if (use_rtc_fg_soc == KAL_TRUE) {
			...
			/* decrease SOC for discharged during booting */
			if (gFG_capacity_by_v < g_rtc_fg_soc) {
				gFG_capacity_by_v = g_rtc_fg_soc - 1;
			} else {
...
void oam_init(void)
{
...
#ifdef CONFIG_MTK_EXTERNAL_CHARGER_SUPPORT
	{
		...
		kal_int32 i = CUST_BOOTING_CURRENT * 10; /* 0.1mA */
	...
	/* HW OCV can be trusted only when boot reason is power-key */
	if (g_boot_reason != BR_POWER_KEY) {
		bm_print(BM_LOG_CRTI,
...
void oam_run(void)
{
	...
	d5_count_time = 60;
#endif
#ifdef CONFIG_LGE_PM_ACCUMULATE_CAR
...
#ifdef CONFIG_LGE_PM_ACCUMULATE_CAR
#if defined(SOC_BY_SW_FG)
static int battery_meter_accumulate_car()
{
	static kal_int32 sleep_time = 0;
	static kal_int32 car = 0;

	sleep_time += _g_bat_sleep_total_time;
	car = (4 * 10 * sleep_time) / 3600; /* standby 4mA */
	...
	/* update soc */
	if (_g_bat_sleep_total_time > d5_count_time)
		bat_spm_timeout = true;

	return bat_spm_timeout;
}
#endif
#endif
...
#ifdef CONFIG_LGE_PM_ACCUMULATE_CAR
#if defined(SOC_BY_SW_FG)
	battery_meter_accumulate_car();

	bm_print(BM_LOG_CRTI,
		 "sleeptime=(%d)s, D0=(%d), car1=(%d), car2=(%d)\n",
		 _g_bat_sleep_total_time, oam_d0, oam_car_1, oam_car_2);
#endif
#else	/* MTK Original */
	if (_g_bat_sleep_total_time < g_spm_timer) {
		return 0;
