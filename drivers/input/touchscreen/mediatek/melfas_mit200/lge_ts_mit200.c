...
static int mit_fw_version_check(struct mms_data *ts, struct touch_fw_info *info)
{
	...
	/*Consider dualization for P5 panel of LCD*/
	if (strcmp(ts->pdata->fw_product, bin_product_id) != 0 && strcmp(ts->pdata->p5_fw_product, bin_product_id) != 0 ) {
		TOUCH_INFO_MSG("F/W Product is not matched [OriginP1:%s][OriginP5:%s][IC:%s][[Bin:%s] \n", 
			ts->pdata->fw_product, ts->pdata->p5_fw_product, ts->module.product_code, bin_product_id);
