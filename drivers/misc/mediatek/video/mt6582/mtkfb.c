...
#if 1 //defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141224 jongwoo82.lee : change LCD FPS */
        lcd_fps = 6000;
#else    
...
void mtkfb_clear_lcm(void)
{
	...
	if(sem_early_suspend_cnt) //sangcheol.seo@lge.com , Slow LCD on Issue.
	{
	if (!lcd_fps)
        msleep(30);
    else
...
static void mtkfb_early_suspend(struct early_suspend *h)
{
    ...
    mutex_lock(&ScreenCaptureMutex);
#if defined(CONFIG_MTK_LEDS)
#if 0 //ndef MTKFB_FPGA_ONLY //sangcheol.seo@lge.com blocking for ESD recovery
    mt65xx_leds_brightness_set(MT65XX_LED_TYPE_LCD, LED_OFF);
#endif
#endif
    if (!lcd_fps)
...
#if 1 //sangcheol.seo@lge.com 20151103 , ESD Recovery
unsigned gbackupAddress;

void mtkfb_display_last(void)
{
	UINT32 fbsize;
	unsigned offset;
	...
	offset = gbackupAddress - (unsigned)fbdev->ovl_pa_base;
	fbsize = ALIGN_TO(DISP_GetScreenWidth(),disphal_get_fb_alignment())*DISP_GetScreenHeight()*3;
	mtkfb_fbi->var.bits_per_pixel = 24;

	memcpy(mtkfb_fbi->screen_base, (unsigned char*)fbdev->ovl_va_base + offset, fbsize);
...
void display_restart(void)
{
	...
	printk("[MTKFB] Display restart START!!! \n");

	is_mtkfb_reset = TRUE;
	if(is_shutdown == TRUE)
	{
		printk("[MTKFB] Shutdown!! No display restart \n");
		 return;
	}

	mtkfb_early_suspend(&h);
	msleep(1); //seosc
	mtkfb_late_resume(&h);

	mtkfb_display_last();

	is_mtkfb_reset = FALSE;
	printk("[MTKFB] Display restart END!!!\n");
}
#endif
