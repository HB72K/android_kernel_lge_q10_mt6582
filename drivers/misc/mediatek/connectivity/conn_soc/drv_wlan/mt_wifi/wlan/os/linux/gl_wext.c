    ...
    )
{
    ...
    /* iwr->u.data.pointer should be SCAN-PASSIVE*/
    if (FALSE == GLUE_CHK_PR2(prNetDev, iwr) ||
        !iwr->u.data.pointer || iwr->u.data.length < 12) {
        return -EINVAL;
    }
    prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));
    ...
    )
{
    ...
    /* iwr->u.data.pointer should be SCAN-ACTIVE*/
    if (FALSE == GLUE_CHK_PR2(prNetDev, iwr) ||
        !iwr->u.data.pointer || iwr->u.data.length < 11) {
        return -EINVAL;
    }
    prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));
...
#endif //#ifdef CONFIG_MTK_TC1_FEATURE //BRIAN TEST
    ...
    )
{
	...
	case SIOCSIWPRIV: /* 0x8B0C, flags 1 : Country, flag2 : passive scan */ 	
		if(iwr->u.data.flags == 0x0001)
			...
			ret = wext_set_passive_scan(prDev, iwr);
		else if(iwr->u.data.flags == 0x0003)
			ret = wext_set_active_scan(prDev, iwr);	
    	break;
#else
