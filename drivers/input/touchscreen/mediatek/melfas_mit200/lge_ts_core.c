...
#include "L0M45P1CY5_30_21_bin.h"          //fab p4
#include "L0M45P5CY5_30_24_bin.h"          //fab p5
...
#if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 150209 ticklewind.kim : YK LGD panel dualization */
extern unsigned int lcm_id;
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 150209 ticklewind.kim : YK LGD panel dualization */
...
/*Select F/W */
	...
	if ( lcm_id == 1 )
	{
		fw_binary = MELFAS_binary;
		fw_length = (size_t)sizeof(MELFAS_binary);
	}
	else
	{
		fw_binary = MELFAS_binary_p5;
		fw_length = (size_t)sizeof(MELFAS_binary_p5);						
	}
#endif
...
static int touch_parse_dt(struct device *dev, struct touch_platform_data *pdata)
{
	...
	struct touch_dt_map map[] = {
		...
		{ "slope_max", &limit->slope_max, DT_U32, 111 },
		{ "slope_min", &limit->slope_min, DT_U32, 89 },
...
//case DT_STRING:
		...
		strcpy(pdata->p5_fw_product , "L0M45P5CY5");

		TOUCH_INFO_MSG("DT entry: pdata->maker = %s \n", pdata->maker);
		TOUCH_INFO_MSG("DT entry: pdata->fw_product = %s \n", pdata->fw_product);
		TOUCH_INFO_MSG("DT entry: pdata->p5_fw_product = %s \n", pdata->p5_fw_product);
#endif //IS_MTK
	if (role->key_type == 0)
		role->key_type = TOUCH_HARD_KEY;
