...
#define AF_DEBUG
#ifdef AF_DEBUG
//#define LOG_INF(format, args...) pr__debug(AF_DRVNAME " [%s] " format, __FUNCTION__, ##args)
#define LOG_INF printk//pr__debug(AF_DRVNAME " [%s] " format, __FUNCTION__, ##args)
#else
...
u8 read_data_zc533(u8 addr)
{
	u8 get_byte = 0;
	i2c_read(addr, &get_byte);
	LOG_INF("[ZC533AF]  get_byte %d\n", get_byte);
	return get_byte;
}


static int zc533AF_ReadReg(unsigned short *a_pu2Result)
{
    int  i4RetValue = 0;
    char pBuff[2];

    *a_pu2Result = (read_data_zc533(0x03) << 8) + (read_data_zc533(0x04) & 0xFF);

    LOG_INF("[ZC533AF]  zc533AF_ReadReg %d\n", *a_pu2Result);
    return 0;
}

static int zc533AF_WriteReg(u16 a_u2Data)
{
    int  i4RetValue = 0;
    ...
    if (i4RetValue < 0)
    {
        LOG_INF("I2C send 2 failed!! \n");
        return -1;
    }

    LOG_INF("[ZC533AF] zc533AF_WriteReg %x, %x\n", puSendCmd1[1], puSendCmd2[1]);
...
inline static int getAFInfo(__user stZC533AF_MotorInfo * pstMotorInfo)
{
    ...
    if (g_zc533AF_Opened >= 1)    {stMotorInfo.bIsMotorOpen = 1;}
...
static int ZC533_init_setting(void)
{

    int  i4RetValue = 0;
    char puSendCmd1[2] = {0x02, 0x00};
    char puSendCmd2[2] = {0x05, 0x06/*0x05*/}; //LGE_CHANGE: [2015-12-01] yonghwan.lym@lge.com, Code for de-noise(ringing)
    char puSendCmd3[2] = {0x06, 0x64/*0x92*/}; //LGE_CHANGE: [2015-12-01] yonghwan.lym@lge.com, Code for de-noise(ringing)

    LOG_INF("[ZC533AF] ZC533_init_setting start\n");
    ...
    LOG_INF("[ZC533AF] ZC533_init_setting end\n");
...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    {
        LOG_INF("[ZC533AF] out of range \n");
        return -EINVAL;
    }

    if (g_zc533AF_Opened == 1)
    {
        unsigned short InitPos;
        ZC533_init_setting();
        ret = zc533AF_ReadReg(&InitPos);

        if(ret == 0)
        {
            LOG_INF("[ZC533AF] Init Pos %6d \n", InitPos);
    ...
    LOG_INF("move [curr] %lx [target] %lx\n", g_u4CurrPosition, g_u4TargetPosition);
            ...
            if(zc533AF_WriteReg((unsigned short)g_u4TargetPosition) == 0)
            {
                spin_lock(&g_AF_SpinLock);
...
{
    LOG_INF("[ZC533AF] Start \n");

    if(g_zc533AF_Opened)
    {
        LOG_INF("[ZC533AF] The device is opened \n");
        return -EBUSY;
    }
...
static int AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if (g_zc533AF_Opened == 2)
    {
        g_sr = 5;
        zc533AF_WriteReg(200);
        msleep(10);
        zc533AF_WriteReg(100);
        msleep(10);
    }
...
MODULE_AUTHOR("BS MOON <byungsoo.moon@lge.com>");
MODULE_LICENSE("GPL");
