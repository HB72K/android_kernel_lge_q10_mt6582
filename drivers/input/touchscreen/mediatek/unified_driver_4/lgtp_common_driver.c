...
static void WqBluReset(struct work_struct *work_blu_reset)
{
	TouchDriverData *pDriverData = container_of(to_delayed_work(work_blu_reset),
		TouchDriverData, work_blu_reset);
	int check_lock = mutex_is_locked(pMutexTouch);
...
static int touch_probe(struct platform_device *platform_dev)
{
	...
	INIT_DELAYED_WORK(&pDriverData->work_blu_reset, WqBluReset);
	...
	if (pDriverData->icFwInfo.isOfficial || !pDriverData->icFwInfo.isOfficial) {
		if (pDriverData->binFwInfo.version != pDriverData->icFwInfo.version) {
			...
			queue_delayed_work(touch_wq, &pDriverData->work_blu_reset, 0);
		}
	}
#if defined ( TOUCH_MODEL_M2 )/* apply temporarily to change GPIO65 mode(BPI_BUS) */
	mt_set_gpio_mode((GPIO65 | 0x80000000), 1);
#endif
