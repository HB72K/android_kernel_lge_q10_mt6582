...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 150126 jongwoo82.lee : YK LGD panel dualization */
#include "dsi_drv.h"
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 150126 jongwoo82.lee : YK LGD panel dualization */
#endif
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 150126 jongwoo82.lee : YK LGD panel dualization */
#define LCM_ID_BY_REGISTER_READ  // if NOT enable the define, using MAKER ID pin(GPIO)

unsigned int lcm_id = 1;  // 1 : default panel, 2 : 2nd panel
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 150126 jongwoo82.lee : YK LGD panel dualization */
...
static struct LCM_setting_table lcm_initialization_setting[] = {
        {0xF0, 15, {0x01, 0x02, 0x04, 0x08, 0x07, 0x08, 0x08, 0x08, 0x08, 0x0C, 0x1A, 0x1B, 0x1C, 0x1E, 0x44}},
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 150126 jongwoo82.lee : YK LGD panel dualization */
// 2nd panel initial code
// gamma 150316
static struct LCM_setting_table lcm_initialization_setting_2nd_panel[] = {
    {0xF0, 15, {0x01, 0x02, 0x04, 0x08, 0x07, 0x08, 0x08, 0x08, 0x08, 0x0C, 0x1A, 0x1B, 0x1C, 0x1E, 0x44}},
    {0xB7, 7, {0x00, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C, 0x1C}},
    {0xB6, 6, {0x06, 0x0A, 0x34, 0x23, 0x41, 0x0A}},
    {0xB8, 4, {0x00, 0x42, 0x12, 0xF7}},
    ...
    {0xC5, 3, {0x3F, 0x00, 0x50}},
    {0xC4, 1, {0x00}},
    {0xD0, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {0xD1, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {0xD2, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {0xD3, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {0xD4, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {0xD5, 9, {0x20, 0x54, 0x55, 0x13, 0x00, 0x00, 0x42, 0x03, 0x02}},
    {REGFLAG_END_OF_TABLE, 0x00, {}},
};
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 150126 jongwoo82.lee : YK LGD panel dualization */
...
{
#if defined(LCM_ID_BY_REGISTER_READ)
	unsigned int data_array[2];
	unsigned char buffer[3] = {0x88,0x88, 0x88};

        DSI_clk_HS_mode(1);  // clk HS mode
	...
	// 2
        ...
        data_array[1] = 0x00FFFFFC;
	dsi_set_cmdq(data_array, 2, 1);
        // 3
        data_array[0] = 0x00023902;
        data_array[1] = 0x00000357;
        dsi_set_cmdq(data_array, 2, 1);
        // 4        
        //read_reg_v2(0xd9, buffer, 3);  // read function test, when read d9h, it returns a0h after init code write
        read_reg_v2(0x04, buffer, 3);  // ID read, 1st panel return all zero

        LCM_PRINT("[LCD] LCM ID READ : [0x%x], [0x%x], [0x%x]\n", buffer[0], buffer[1], buffer[2]);

        DSI_clk_HS_mode(0);  // clk LP mode

        // condition to check 2nd panel id
        if( buffer[0] == 0x00 && buffer[1] == 0x05 && buffer[2] == 0x00)  // need to change the condition for real dualization panel, now test Panel ID
        {
            LCM_PRINT("[LCD] LGD panel type : P5\n");
            return 2;
        }

        LCM_PRINT("[LCD] LGD panel type : P4 default\n");
        return 1;  // default 1st panel
#else  // lcm id by maker_ID
        return 1;  // default ID
#endif
}
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 150126 jongwoo82.lee : YK LGD panel dualization */
static void init_lcm_registers_2nd(void)
{
    #if (!defined(BUILD_UBOOT) && !defined(BUILD_LK))  && defined(INIT_CODE_TUNNING_BY_SD_CARD)  /* LGE_BSP_COMMON LGE_CHANGE_S 141105 jongwoo82.lee : init code tunning */
         //sd_initial
        ...
        LCM_PRINT("[LCD] : [KERNEL] init_lcm_registers 2nd\n");
        if (Filesize(LCD_INITIAL_TXT) > 0)
        {
            line_cnt= lcd_init_read_from_sd();
            if (line_cnt > 0)
            {
                ...
                LCM_PRINT("[LCD] : Default initial setting\n");
                push_table(lcm_initialization_setting_2nd_panel, sizeof(lcm_initialization_setting_2nd_panel) / sizeof(struct LCM_setting_table), 1);
            }
        }
        else
        {
    ...
    #else
        push_table(lcm_initialization_setting_2nd_panel, sizeof(lcm_initialization_setting_2nd_panel) / sizeof(struct LCM_setting_table), 1);
        LCM_PRINT("[LCD] init_lcm_registers 2nd\n");
    #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141105 jongwoo82.lee : init code tunning */
}
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 150126 jongwoo82.lee : YK LGD panel dualization */
...
static void lcm_init(void)
{
        ...
        if(lcm_id != 2)  // default lcm
        {
            init_lcm_registers();
        }
        else  // 2nd lcm
        {
            init_lcm_registers_2nd();
        }
