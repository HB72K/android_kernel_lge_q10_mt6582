...
#endif//COMMON_CAM_CAL_DRV
{
    ...
    {
        kfree(pBuff);
        EEPROMDB("DW9716EEPROM] ioctl allocate mem failed\n");
        return -ENOMEM;
    }
     EEPROMDB("[DW9716EEPROM] init Working buffer address 0x%8x  command is 0x%8x\n", (u32)pWorkingBuff, (u32)a_u4Command);
    ...
    if(copy_from_user((u8*)pWorkingBuff ,  (u8*)ptempbuf->pu1Params, ptempbuf->u4Length))
    {
        kfree(pBuff);
        kfree(pWorkingBuff);
        EEPROMDB("[DW9716EEPROM] ioctl copy from user failed\n");
        return -EFAULT;
    } 
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:    
            EEPROMDB("[DW9716EEPROM] Write CMD \n");
#ifdef EEPROMGETDLT_DEBUG
        ...
        case CAM_CALIOC_G_READ:
            EEPROMDB("[DW9716EEPROM] Read CMD \n");
#ifdef EEPROMGETDLT_DEBUG            
            do_gettimeofday(&ktv1);
#endif 
            EEPROMDB("[DW9716EEPROM] offset %x \n", ptempbuf->u4Offset);
            EEPROMDB("[DW9716EEPROM] length %x \n", ptempbuf->u4Length);
            EEPROMDB("[DW9716EEPROM] Before read Working buffer address 0x%8x \n", (u32)pWorkingBuff);
            ...
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000d9716) && (EEPROM_Address[0]==0x0))
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
                ...
                EEPROMDB("[DW9716EEPROM] Init I2C address %x \n", *(u32 *)pWorkingBuff );
            }
            else if( ((ptempbuf->u4Offset & 0x000FFFFF) == 0x000d9716) && (ResetCheck == 0xABCDFEDC) )
            {
                *(u32 *)pWorkingBuff = (ptempbuf->u4Offset | 0x10000000);
	
                
                EEPROMDB("[DW9716EEPROM] Reset I2C address %x \n", *(u32 *)pWorkingBuff );
            }
            else
            {
               readTryagain=3;
            ...
            EEPROMDB("[DW9716EEPROM] After read Working buffer data  0x%4x \n", *pWorkingBuff);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        EEPROMDB("[DW9716EEPROM] to user length %d \n", ptempbuf->u4Length);
        EEPROMDB("[DW9716EEPROM] to user  Working buffer address 0x%8x \n", (u32)pWorkingBuff);
