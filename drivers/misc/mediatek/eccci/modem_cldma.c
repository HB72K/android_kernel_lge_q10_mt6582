...
static int cldma_rx_collect(struct md_cd_queue *queue, int budget, int blocking, int *result)
{
    ...
    *result = UNDER_BUDGET;
    ...
    while(1) { // not hardware own
        ...
        // update skb
        dma_unmap_single(&md->plat_dev->dev, rgpd->data_buff_bd_ptr, skb_data_size(req->skb), DMA_FROM_DEVICE);
...
#endif
            if(likely(req->skb)) {
                rgpd->data_buff_bd_ptr= dma_map_single(&md->plat_dev->dev, req->skb->data, skb_data_size(req->skb), DMA_FROM_DEVICE);
                // checksum of GPD
...
static void cldma_tx_done(struct work_struct *work)
{
    ...
    while(1) {
        ...
        // update counter
        queue->free_slot++;
        dma_unmap_single(&md->plat_dev->dev, tgpd->data_buff_bd_ptr, req->skb->len, DMA_TO_DEVICE);
...
static void cldma_rx_queue_init(struct md_cd_queue *queue)
{
    ...
    for(i=0; i<rx_queue_buffer_number[queue->index]; i++) {
        ...
        gpd->data_buff_bd_ptr = dma_map_single(&md->plat_dev->dev, req->skb->data, skb_data_size(req->skb), DMA_FROM_DEVICE);
...
retry:
    ...
    if(queue->free_slot > 0) {
        ...
        ccci_free_req(req);
        // update GPD
        tgpd->data_buff_bd_ptr = dma_map_single(&md->plat_dev->dev, tx_req->skb->data, tx_req->skb->len, DMA_TO_DEVICE);
		...
		if(cldma_read32(md_ctrl->cldma_ap_base, CLDMA_AP_UL_STATUS) & (1<<qno))
			queue->busy_count++;
		if(likely(md->capability & MODEM_CAP_TXBUSY_STOP)) 
...
static int md_cd_send_runtime_data(struct ccci_modem *md, unsigned int sbp_code)
{
    ...
    // SBP
#if 1 //def MTK_MD_SBP_CUSTOM_VALUE
	#if 0 
    if (sbp_code > 0) {
	...
	else
	{
		...
		memcpy((char*)&(runtime->feature_4_val[3]), temp_buf_svn, 4); /* SVN */

		printk("jjm_debug : md_cd_send_runtime_data : runtime.feature_4_val[3] = %s\n", (char *)(&runtime->feature_4_val[3]));
	...
	else
	{
		...
		if (result == true)
		{
			printk("jjm_debug : md_cd_send_runtime_data : Read LGE_FacReadNetworkCode_SBP Success!\n");
		}
		else
		{
			printk("jjm_debug : md_cd_send_runtime_data : Read Fail! result = %d\n", result);
		}

		lge_convert_ntcode_format_to_string (temp_buf, mcc, mnc, subset);

		printk("jjm_debug : md_cd_send_runtime_data : mcc_out = %s, mnc_out = %s, subset_out = %s\n", mcc, mnc, subset);

		kfree(temp_buf);

		memcpy((char*)&(runtime->feature_4_val[0]), mcc, 4); /* MCC */
		memcpy((char*)&(runtime->feature_4_val[1]), mnc, 4); /* MNC */
		memcpy((char*)&(runtime->feature_4_val[2]), subset, 4); /* Subset */

		runtime->support_mask |= (FEATURE_SUPPORT<<(MISC_MD_SBP_SETTING*2));

		printk("jjm_debug : md_cd_send_runtime_data : runtime.feature_4_val[0] = %s, runtime.feature_4_val[1] = %s, runtime.feature_4_val[2] = %s", (char *)(&runtime->feature_4_val[0]), (char *)(&runtime->feature_4_val[1]), (char *)(&runtime->feature_4_val[2]));
	}

	memcpy((char*)&(runtime->feature_8_val[0]), g_lge_sim_num, 4); /* SIM count */
	runtime->support_mask |= (FEATURE_SUPPORT<<(MISC_MD_SBP_SETTING_2*2));

	printk("jjm_debug : md_cd_send_runtime_data : runtime.feature_4_val[0] = %s, runtime.feature_4_val[1] = %s, runtime.feature_4_val[2] = %s, runtime.feature_4_val[3] = %s, runtime.feature_8_val[0] = %s\n", (char *)(&runtime->feature_4_val[0]), (char *)(&runtime->feature_4_val[1]), (char *)(&runtime->feature_4_val[2]), (char *)(&runtime->feature_4_val[3]), (char *)(&runtime->feature_8_val[0]));
	/* sbp_interface */
	#endif
#endif
    // CCCI debug
