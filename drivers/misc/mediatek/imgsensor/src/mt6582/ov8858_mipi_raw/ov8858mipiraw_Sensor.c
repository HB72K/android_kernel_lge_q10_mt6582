...
#include <linux/slab.h> //LGE_CHANGE: [2015-12-15] yonghwan.lym@lge.com, OV8858 OTP Enable
...
static void OV8858_read_otp_lenc(void)
{
	...
	OV8858DB("OV8858_read_otp_lenc Start\n");

	if(pOTP_Data != NULL) {
		OV8858DB("[OV8858_read_otp_lenc] OTP LenC Data already read\n");
		return;
	}

	pOTP_Data = kzalloc(sizeof(kal_uint8)*240, GFP_KERNEL);
	if(pOTP_Data == NULL) {
		OV8858DB("[OV8858_read_otp_lenc] Memory allocation fail\n");
		return;
	}

	// Disable OTP_DPC , set 0x5002[3] to "0"
	temp = OV8858_read_cmos_sensor(0x5002);
	OV8858_write_cmos_sensor(0x5002, (temp & (~0x08)));

	OV8858_write_cmos_sensor(0x3d84,0xc0);

	OV8858_write_cmos_sensor(0x3d88,(otp_start_addr>>8)&0xff);
	OV8858_write_cmos_sensor(0x3d89,otp_start_addr & 0xff);
	OV8858_write_cmos_sensor(0x3d8a,(otp_end_addr>>8)&0xff);
	OV8858_write_cmos_sensor(0x3d8b,otp_end_addr & 0xff);
	...
	// Check Flag
	flag = OV8858_read_cmos_sensor(0x7011);
	...
	if(flag == 1){
		...
		OV8858DB("[OV8858_read_otp_lenc] flag_lenc error : %d\n",flag);
		if(pOTP_Data != NULL){
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
	...
	for(loop=0; loop < 240; loop++) {
		pOTP_Data[loop]= (OV8858_read_cmos_sensor(start_addr+loop) & 0xFF);
		checksum += pOTP_Data[loop];
	}

	// Read LenC Checksum
	ReadCheckSum = ((OV8858_read_cmos_sensor(end_addr+1)&0xFF)  |  ((OV8858_read_cmos_sensor(end_addr+2)&0xFF) << 8));
	OV8858DB("[OV8858_read_otp_lenc] checksum = 0x%x, ReadCheckSum = 0x%x\n",checksum,ReadCheckSum);

	if(checksum != ReadCheckSum) {
		OV8858DB("[OV8858_read_otp_lenc] checksum error\n");
		if(pOTP_Data != NULL) {
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
	...
	OV8858DB("OV8858_read_otp_lenc End\n");
	return;
}
static void OV8858_write_otp_lenc(void)
{
	kal_uint16 loop=0, temp=0;
	OV8858DB("OV8858_write_otp_lenc Start\n");

	// Write LenC Data
	for(loop=0; loop < 240; loop++) {
		OV8858_write_cmos_sensor(0x5800+loop , pOTP_Data[loop]);
	}

	// apply the LenC , set 0x5000[7] to "1"
	temp = OV8858_read_cmos_sensor(0x5000);
	OV8858_write_cmos_sensor(0x5000, (temp | 0x80));

	OV8858DB("OV8858_write_otp_lenc End\n");
	return;
}
...
static void OV8858_read_otp_awb(void)
{
	...
	OV8858DB("OV8858_read_otp_awb Start\n");
	...
	if(flag == 1){
		...
		OV8858DB("[OV8858_read_otp_awb] flag error : %d\n",flag);
		return;
	}
	...
	for(loop=0; loop < 3; loop++) {
		...
		temp1 = (OV8858_read_cmos_sensor(start_addr+(loop*2)) &0xFF);
		temp2 = (OV8858_read_cmos_sensor(start_addr+(loop*2)+1) &0xFF);
	...
	// Read LenC Checksum
	ReadCheckSum = ((OV8858_read_cmos_sensor(end_addr+1) & 0xFF)  |  ((OV8858_read_cmos_sensor(end_addr+2)& 0xFF) << 8));
	OV8858DB("[OV8858_read_otp_awb] checksum = 0x%x, ReadCheckSum = 0x%x\n",checksum,ReadCheckSum);

	if(checksum != ReadCheckSum) {
		OV8858DB("[OV8858_read_otp_awb] checksum error\n");
		gOTP_AWB_Data[0] = 0;
		gOTP_AWB_Data[1] = 0;
		gOTP_AWB_Data[2] = 0;
		return;
	...
	OV8858DB("OV8858_read_otp_awb End\n");
}
...
#endif
#if defined(TARGET_MT6582_M23G)
extern int module_flag_m23g; // 1:5MP(OV5670), 0:8MP(OV8858)
#endif
...
{
// [LGE_UPDATE_S][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90
#if defined(TARGET_MT6582_Y90)// || defined(TARGET_MT6582_M23G)
    if(module_flag != 0)
    {
...
#endif
#if defined(TARGET_MT6582_M23G)
	if(module_flag_m23g == 1)
    {
        printk("Please check the flag and module. module_flag = %d ...(flag 1 => 5MP, flag 0 => 8MP)\n",module_flag_m23g);
        *sensorID = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
    }
#endif
...
//Gionee yanggy modify for CAM Performance Optimization begin
    ...
    pSensorInfo->PreviewDelayFrame = 3;//1; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-03-27] occurs "purple noise" after flash capture
#endif
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                *pHorFOV = (0x43)<<16 | (0x43); // 16:9 = 67, 4:3 = 67 // (67)<<16 | (67)
