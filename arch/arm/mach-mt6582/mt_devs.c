...
#if defined (TARGET_MT6582_Y50) || defined(TARGET_MT6582_M23G)/* ONE_BIN_MEMORY */ /* M23G_ONE_BIN_MEMORY */
...
void mt_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
...
#if defined (TARGET_MT6582_M23G) /* M23G_ONE_BIN_MEMORY */
    unsigned int lge_reserved_ram_check=0;
#endif
...
#if defined (TARGET_MT6582_M23G) /* M23G_ONE_BIN_MEMORY */
		if(tags->u.mem.size == RANK_SIZE_1_5GB){ //LGE_BSP_MEM_MAX_PHY_ADDR : 0xf000 0000
			LGE_BSP_MEM_MAX_PHY_ADDR =(LGE_BSP_MEM_BASE_ADDR + tags->u.mem.size + bl_mem_sz + PROTECTION_RAM_SIZE);

			if(lge_reserved_ram_check){
		   		tags->u.mem.size -=0x300000; // LGE_BSP_MEM_MAX_PHY_ADDR(0xf000 0000) - 0x300000(3MB) : Reserve Area
			}

			lge_reserved_ram_check =1; // just excute one time 
		}
		else{ //LGE_BSP_MEM_MAX_PHY_ADDR : 0xc000 0000
			LGE_BSP_MEM_MAX_PHY_ADDR =(LGE_BSP_MEM_BASE_ADDR + tags->u.mem.size);
		   	tags->u.mem.size -=0x300000; // LGE_BSP_MEM_MAX_PHY_ADDR(0xc000 0000) - 0x300000(3MB) : Reserve Area
		}
		printk(KERN_ALERT "MEM_SIZE : 0x%llx\n", (unsigned long long)tags->u.mem.size);
              printk(KERN_ALERT "LGE_BSP_MEM_MAX_ADDR : 0x%llx\n", (unsigned long long)LGE_BSP_MEM_MAX_PHY_ADDR);
#endif	
    ...
    MTK_MEMCFG_LOG_AND_PRINTK(KERN_ALERT
            ...
            #endif
            #if defined (TARGET_MT6582_M23G) /* M23G_ONE_BIN_MEMORY */
            "[PHY layout]Max PHY ADDR (lk) = 0x%llx\n"
            #endif
...
#if defined (CONFIG_BACKLIGHT_LM3639) || defined (CONFIG_BACKLIGHT_LM3632) || defined (CONFIG_CUSTOM_KERNEL_CHARGEPUMP) || defined(CONFIG_LEDS_RT4832)
...
struct platform_device power_sysfs_dev = {
    .name   = "power-sysfs",
    .id     = -1,
};
#endif
...
/* LGE_CHANGE: sangcheol.seo@lge.com 20140925 : add backlight device as charge pump*/
#if defined (CONFIG_BACKLIGHT_LM3639) || defined (CONFIG_BACKLIGHT_LM3632) || defined(CONFIG_LEDS_RT4832)
...
#ifdef CONFIG_MACH_LGE
    do {
        retval = platform_device_register(&power_sysfs_dev);
        if (retval != 0) {
            return retval;
        }
    } while (0);
