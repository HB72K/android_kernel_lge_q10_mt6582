...
#define BMM050_BUFSIZE  0x2C

#define MSE_TAG					"[LGE_Magnetometer] "
...
static void bmm050_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	MSE_LOG("bmm050_setPowerMode enable : %d\n",enable);
	if (enable == FALSE)
	{
...
static long bmm050_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case ECOMPASS_IOC_SET_YPR:				//used by daemon
			...
			ECS_SaveData(value);
           /* read data count here for at%motion=2 */
            Count++;
				if(Count>1000000){
					Count = 0;
				}
		...
		case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:		//used by MTK ftm
			...
			osensor_data->values[0] = sensor_data[8]/CONVERT_O_DIV;
			osensor_data->values[1] = sensor_data[9]/CONVERT_O_DIV;
			osensor_data->values[2] = sensor_data[10]/CONVERT_O_DIV;
...
//	obj->odr = BMM050_HIGHACCURACY_DR;
...
static int __init bmm050_init(void)
{
	MSE_FUN();
	struct mag_hw *hw = get_cust_mag_hw();
		unsigned short bmm050_i2c_addr = 0x0;

#if defined(TARGET_MT6582_Y50)||defined(TARGET_MT6582_Y70)
		MSE_LOG("forced bmm050_init addr change\n");
		bmm050_i2c_addr = 0x12;
