...
/* sangcheol.seo@lge.com LGD in-cell Sleep sequence */
extern void init_lcm_registers_sleep_2nd(void);
...
//sangcheol.seo@lge.com 20151103, Improve LCD on time
...
static void lcm_initTimer_for_mdelay()
{
...
#define VSYNC_MS_TO_NS(x)¢®i       (x * 1E6L)

static void lcm_mdelay(UINT32 ms)
{
#if 0 //improve LCD on time
    msleep(ms);
#else
...
int disphal_panel_enable(const LCM_DRIVER *lcm_drv, struct mutex* pLcmCmdMutex, BOOL enable)
{
    ...
    else
    {
		...
		/* sangcheol.seo@lge.com LGD in-cell Sleep sequence */
#if 0 /* move to Touch FW (20151228) */
		if (!is_shutdown)
		{
			mutex_lock(pLcmCmdMutex);
			init_lcm_registers_sleep_2nd();
			mutex_unlock(pLcmCmdMutex);
		 }
#endif
        }
