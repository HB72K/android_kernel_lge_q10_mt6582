...
//sangcheol.seo@lge.com 20151130, MTK Aden.lee's patch for ESD recovery
	while(_IsEngineBusy()) {
		msleep(1);
		if (--timeOut < 0)	{

			DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DSI", " Wait for DSI engine not busy timeout!!!(Wait %d us)\n", get_current_time_us() - time);
			printk("SEOSCTEST: ESD");
			DSI_DumpRegisters();
			DSI_Reset();
...
//sangcheol.seo@lge.com 20151130
    }
...
void DSI_change_mode(DSI_STATUS_FOR_MIX_MODE status)
{
    ...
    switch (status)
    {
        ...
        case DSI_INCELL_SEND_FRAME:
            ...
            {                
                int cnt_sleep = 0;
			#if 1 //defined(TARGET_MT6582_P1S3G)
				dsi_incell_mix_mode_status = DSI_INCELL_SUSPEND_MODE;
...
void DSI_MixMode_Start(void)
{
    ...
    if (dsi_incell_mix_mode_status == DSI_INCELL_IDLE_MODE)
    {
        ...
        dsi_single_frame_mode_timer.function = _DSI_Send_Frame_without_TE_handler;
        if (lcm_params->dsi.PLL_CLOCK != lcm_params->dsi.mixmode_mipi_clock) //20151022 mtk patch
        {
        OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,VM_DONE,1);
        DSI_Wait_VDO_Idle();
		}
...
//		pmod = 433.33;
	...
	OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_PHI_SEL,0x1);
        #if defined(TARGET_MT6582_Y50) || defined(TARGET_MT6582_M23G)  /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
		//sangcheol.seo  151207 : LGD with DB7400 IC has brocken screen issue
	...
	if(lcm_params->dsi.LANE_NUM > 2)
	{
		...
		#if defined(TARGET_MT6582_M23G) /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
		//sangcheol.seo  151207 : LGD with DB7400 IC has brocken screen issue
		OUTREGBIT(MIPITX_DSI0_DATA_LANE2_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE2,RG_DSI0_LNT2_LPTX_IPLUS1,0x1); // +10% for P
		OUTREGBIT(MIPITX_DSI0_DATA_LANE2_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE2,RG_DSI0_LNT2_LPTX_IPLUS2,0x0); // normal for N //0x1, +10% for N
		#endif	/* LGE_BSP_COMMON LGE_CHANGE_E 141127 jongwoo82.lee : LCD up/down shaking */
	}
	...
	if(lcm_params->dsi.LANE_NUM > 3)
	{
		...
		#if defined(TARGET_MT6582_M23G) /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
		//sangcheol.seo  151207 : LGD with DB7400 IC has brocken screen issue
		OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3,RG_DSI0_LNT3_LPTX_IPLUS1,0x1); // +10% for P
		OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3,RG_DSI0_LNT3_LPTX_IPLUS2,0x0); // normal for N //0x1, +10% for N
		#endif	/* LGE_BSP_COMMON LGE_CHANGE_E 141127 jongwoo82.lee : LCD up/down shaking */
